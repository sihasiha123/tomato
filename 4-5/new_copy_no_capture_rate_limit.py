...
保留检测、对轨、CAN 和工作流逻辑；
不在对轨成功后停推理，但支持通过环境变量开关 YOLO 限频。
"""
...
class FusionControllerNoCapture(BaseFusionController):
    def __init__(self, *args, **kwargs):
        ...
        self.wait_nav_infer_enabled = (
            os.environ.get("RAIL_WAIT_NAV_INFER_ENABLE", "1").strip().lower()
            in {"1", "true", "yes", "on"}
        )
        self.yolo_limit_enabled = (
            os.environ.get("RAIL_YOLO_LIMIT_ENABLE", "1").strip().lower()
            in {"1", "true", "yes", "on"}
        )
        default_yolo_max_fps = "8.0"
        self.yolo_max_fps = max(
            0.0,
            float(os.environ.get("RAIL_YOLO_MAX_FPS", default_yolo_max_fps)),
        )
        if not self.yolo_limit_enabled:
            self.yolo_max_fps = 0.0
        ...
        rospy.loginfo(
            "🤖 YOLO 运行策略(连续检测版): "
            f"wait_nav_infer={'on' if self.wait_nav_infer_enabled else 'off'}, "
            f"limit={'on' if self.yolo_limit_enabled else 'off'}, "
            f"max_fps={self.yolo_max_fps:.1f}"
        )
        rospy.loginfo("📷 已切换到无采图连续检测版本: 不保存原图，对轨成功后不断流")
        ...
        try:
            while not rospy.is_shutdown():
                ...
                if current_state == SystemState.ALIGNED and not workflow_busy:
                    rospy.loginfo_throttle(2.0, "✅ 对轨已成功，触发工作流，检测保持运行")
